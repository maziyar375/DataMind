...
def test_runs_either_side_of_the_fix_carry_different_prompt_versions() -> None:
    ...
    # earlier) from a score taken with the layer (v10 on). D11. v11 and v12
    # moved it again, for the layer and then the schema's prose indexing
    # retrieval — still layer-on, so the split this test names is unaffected.
