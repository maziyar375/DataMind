...
| run | what it pins |
|---|---|
| analytical, clean | the twelve-node order, `match`/`scope`/`describe`/`clarify` writing SKIPPED rows, and the KPI branch that runs *before* any model call |
...
# The six nodes every run walks before any SQL exists. `match`, `scope`,
# `describe` and `clarify` are SKIPPED here and that is the point: a skipped
# node still writes
...
PREAMBLE = [
    ...
    started(3, "scope"), finished(3, "scope", StepStatus.SKIPPED),
    started(4, "retrieve"), finished(4, "retrieve"),
    started(5, "describe"), finished(5, "describe", StepStatus.SKIPPED),
    started(6, "clarify"), finished(6, "clarify", StepStatus.SKIPPED),
]
...
@pytest.mark.asyncio
async def test_a_clean_analytical_run_emits_the_twelve_node_trail() -> None:
    ...
    assert rows(recorder) == [
        ...
        (3, "scope", StepStatus.SKIPPED),
        (4, "retrieve", StepStatus.DONE),
        (5, "describe", StepStatus.SKIPPED),
        (6, "clarify", StepStatus.SKIPPED),
        (7, "generate", StepStatus.DONE),
        (8, "validate", StepStatus.DONE),
        (9, "execute", StepStatus.DONE),
        (10, "inspect", StepStatus.DONE),
        (11, "present", StepStatus.DONE),
        (12, "chart", StepStatus.DONE),
    ]
    ...
    assert calls(recorder)[:8] == [
        ...
        (2, "match", StepStatus.SKIPPED),
        # `scope` with no sections in the deps: SKIPPED, no model call, and
        # still both rows — the same promise `match` keeps above.
        (3, "scope", StepStatus.RUNNING),
        (3, "scope", StepStatus.SKIPPED),
        (4, "retrieve", StepStatus.RUNNING),
        (4, "retrieve", StepStatus.DONE),
    ]
    assert len(calls(recorder)) == 24
...
async def test_a_budget_cut_is_counted_in_the_retrieve_step(
    ...
    assert trail(recorder)[:len(PREAMBLE)] == PREAMBLE
...
async def test_a_failed_check_retry_restores_forward_into_present() -> None:
    ...
    assert rows(recorder) == [
        ...
        (9, "execute", StepStatus.DONE),
        (10, "inspect", StepStatus.DONE),
        (11, "generate", StepStatus.DONE),
        (12, "validate", StepStatus.DONE),
        (13, "present", StepStatus.DONE),
        (14, "chart", StepStatus.SKIPPED),
    ]
...
async def test_a_rejected_statement_repairs_and_charts() -> None:
    ...
    assert trail(recorder) == PREAMBLE + [
        ...
        finished(11, "execute"),
        started(12, "inspect"), finished(12, "inspect"),
        started(13, "present"),
        event("TEXT_DELTA", "Revenue was $1.24M."),
        finished(13, "present"),
        started(14, "chart"),
    ...
    assert [name for _seq, name, _status in rows(recorder)] == [
        "route", "match", "scope", "retrieve", "describe", "clarify",
...
async def test_a_database_error_repairs_and_then_restores_from_execute() -> None:
    ...
    assert trail(recorder) == PREAMBLE + [
        ...
        started(9, "execute"), finished(9, "execute"),
        started(10, "generate"),
        event("SQL_GENERATED", "attempt 2"),
...
async def test_a_node_crash_is_a_failed_step_and_not_an_exception() -> None:
    ...
    assert trail(recorder)[-2:] == [
        started(9, "execute"),
        finished(9, "execute", StepStatus.FAILED),
    ]
    assert rows(recorder)[-1] == (9, "execute", StepStatus.FAILED)
...
async def test_a_run_that_will_not_converge_is_stopped_not_raised() -> None:
    ...
    # Six preamble nodes now, so the cycle gets nineteen executions rather than
    # twenty: the ceiling counts *node executions*, and `match` and then
    # `scope` each spend one of them on every run. That is the honest
    # consequence of adding a node to the chain, recorded here rather than
    # absorbed by loosening the assertion — a repair budget this large is a
    # runaway either way.
    assert [name for _seq, name, _status in rows(recorder)][6:] == (
        ["generate", "validate"] * 10
    )[:19]
    assert state.error is not None
