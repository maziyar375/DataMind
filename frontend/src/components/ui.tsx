...
export const Icon = {
  ...
  // ── chart glyphs ──
  // Each one is a miniature of the chart it selects, not a symbol standing in
  // for it, because the picker's whole argument is that a name alone ("Combo",
  // "Area") is a guessing game. They share a baseline at y=21 wherever the
  // real chart has one, so the row reads as a set rather than nine drawings.
  Bar: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      <path d="M7 21V11M12 21V4M17 21v-6" />
    </svg>
  ),
  Line: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      <path d="M4 16l5-5 4 3 6-8" />
    </svg>
  ),
  Area: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      <path d="M4 17l5-6 4 3 6-7v11H4z" />
    </svg>
  ),
  Combo: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      <path d="M7 21v-6M12 21v-9M17 21v-4" />
      <path d="M4 10l5-4 4 4 6-5" />
    </svg>
  ),
  Scatter: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      <circle cx="7" cy="15" r="1.5" /><circle cx="11" cy="9" r="1.5" />
      <circle cx="15" cy="16" r="1.5" /><circle cx="19" cy="6" r="1.5" />
    </svg>
  ),
  Pie: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <circle cx="12" cy="12" r="9" />
      <path d="M12 3v9l6.4 6.4" />
    </svg>
  ),
  Heatmap: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <rect x="3" y="3" width="18" height="18" rx="2" />
      <path d="M9 3v18M15 3v18M3 9h18M3 15h18" />
      {/* One hot cell, so the grid reads as a measure and not as a table. */}
      <rect x="9" y="9" width="6" height="6" fill={stroke} stroke="none" />
    </svg>
  ),
  Histogram: ({ size = 17, stroke = 'currentColor', strokeWidth = 2 }: IconProps) => (
    <svg {...iconBase(size, stroke, strokeWidth)}>
      <path d="M3 21h18" />
      {/* Touching, and rising then falling — a distribution, not a comparison,
          which is the whole difference from `Bar` above. */}
      <path d="M5 21v-4M9 21v-9M13 21v-13M17 21v-7M21 21v-3" />
    </svg>
  ),
}
...
// ── the chart picker ──────────────────────────────────────────────────────
/**
 * One entry as the backend states it. `supported: false` always carries a
 * `reason`, which is written server-side because it is made of two things only
 * that side knows: what the type requires, and what this result actually is.
 */
 ...
 *
 * Not the backend's order: that one is written for a model reading a list, and
 * groups by what the reader gets. This one is written for an eye scanning a
 * row, so the three everyday shapes come first and the specialists follow.
 */
const CHART_KINDS: {
  type: string
  label: string
  ...
  { type: 'bar', label: 'Bar', icon: Icon.Bar },
  { type: 'line', label: 'Line', icon: Icon.Line },
  { type: 'area', label: 'Area', icon: Icon.Area },
  { type: 'pie', label: 'Pie', icon: Icon.Pie },
  { type: 'scatter', label: 'Scatter', icon: Icon.Scatter },
  { type: 'combo', label: 'Bar + line', icon: Icon.Combo },
  { type: 'heatmap', label: 'Heatmap', icon: Icon.Heatmap },
  { type: 'histogram', label: 'Histogram', icon: Icon.Histogram },
]

/**
 * Pick a chart type from a grid of miniatures, with the impossible ones out.
 *
 * Two decisions worth knowing about:
 *
 * * **An unsupported type is shown, not hidden.** A list that shrank between
 *   one result and the next would look like a bug in the app rather than a
 *   fact about the data, and the reason on hover is the only place a user ever
 *   learns *why* a heatmap needs two dimensions.
 * * **It is `aria-disabled`, never `disabled`.** A disabled button swallows
 *   pointer events in every browser, which takes the `title` tooltip with it —
 *   and the tooltip is the entire point of drawing the option at all. So the
 *   click is refused in the handler instead.
 *
 * An empty `options` means *nothing is known yet* — a draft whose SQL has not
 * been run — and everything stays enabled. Withholding the picker until a
 * preview exists would be the editor deciding it knows better than a user who
 * has already chosen what they want to build.
 */
export function ChartTypePicker({
  value, options, onChange, auto = false, busy = null,
}: {
  value: string
  options: ChartOption[]
  onChange: (type: string) => void
  /** Offer "Auto" — right for a tile, whose data changes shape between runs. */
  auto?: boolean
  /** The type currently being drawn, if a round trip is in flight. */
  busy?: string | null
}) {
  const verdict = new Map(options.map((o) => [o.type, o]))
  const entries = auto
    ? [{ type: 'auto', label: 'Auto', icon: Icon.Sparkle }, ...CHART_KINDS]
    : CHART_KINDS
  ...
  return (
    ...
    >
      {entries.map(({ type, label, icon: Glyph }) => {
        // Auto is never refused: it means "you decide", and the backend always
        // has an answer to that — including "nothing", which it says out loud.
        const state = type === 'auto' ? undefined : verdict.get(type)
        const allowed = state === undefined || state.supported
        const selected = value === type
        const pending = busy === type
        return (
          <HoverButton
            key={type}
            role="radio"
            ...
            title={allowed ? undefined : state?.reason ?? undefined}
            onClick={() => { if (allowed && !pending) onChange(type) }}
            baseStyle={{
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              ...
              opacity: allowed ? (pending ? 0.6 : 1) : 0.38,
              transition: 'border-color .12s ease, background .12s ease',
            }}
            hoverStyle={allowed && !selected
              ? { borderColor: 'var(--border-strong)', color: 'var(--text)' }
              : {}}
          >
            {pending ? <Spinner size={17} /> : <Glyph size={17} />}
            <span style={{ fontSize: 10, lineHeight: 1.2, letterSpacing: 0.1 }}>
              {label}
            </span>
          </HoverButton>
        )
