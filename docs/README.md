...
| [hybrid-retrieval.md](plans/hybrid-retrieval.md) | **Live — Phases 1–2 of 3 done.** mvp2 **B2**, the last unbuilt Tier 1 row. Phase 1 (2026-09-23) made the schema's *prose* rank tables — DDL comments and the semantic layer's descriptions, IDF-weighted against the question — on the one branch that has to choose; `PROMPT_VERSION` → v12. Phase 2 (`0039`) added one vector per table over that same text, blended `max(lexical, cosine)` and gated on a pinned embedding model, with its own worker and its own eval arm. Phase 3 is what the index says about itself. Two divergences from mvp2 §B2 are argued in §0.2: **no `pgvector`**, and `include_db_comments` governing ranking as well as rendering. §11 is the ledger, and **the arm it owes is unrun** |
