...
| [user-management-and-access-control-plan.md](user-management-and-access-control-plan.md) | **Proposed, not built. The current plan of record for authorization.** Users, **service users**, roles carrying capabilities and wildcard privileges, teams, and grants on eight resource types — argued from [research/access-control.md](research/access-control.md), from a fresh reading of the codebase, and from new external research into Power BI, Superset, Metabase, Grafana, Looker and Tableau. Eleven phases, each with its own gate and acceptance criteria, and a master checklist at the foot. Read §0.4 first: eighteen decisions, four of which reverse `access-control-plan.md`. |
| [access-control-plan.md](access-control-plan.md) | **Superseded, and kept as the argument.** Users, groups, roles and grants, argued from [research/access-control.md](research/access-control.md). Nothing in it ships — the product is still owner-only, as [dashboards.md §9](dashboards.md) and [reports.md §14](reports.md) both say. Its port, its lattice, its intersection rule and its OIDC recipe all survive into the plan above; §0.4 of that plan names the four decisions it reverses. Read §0 of the research note first: it corrects two things this repo believes about its own authorization. |
