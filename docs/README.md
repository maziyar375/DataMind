...
| [semantic-layer-model.md](plans/semantic-layer-model.md) | **Live.** The semantic layer as a model rather than a blob: one binder for every reader (Phase 0, landed), numbered versions with typed changes, a revision check and restore (Phase 1, landed), then draft and publish, metric attribution, export and upkeep. §14 is the ledger |
