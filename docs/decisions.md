...
| **An export carries no values from the data unless asked, and an import lands in the draft** — import is typing by another route, not a guard entry point (D9) | Standing (2026-09-17) | [plans/semantic-layer-model.md](plans/semantic-layer-model.md) §4.5 |
| **Fill the gaps never overwrites a field a person wrote and never touches an existing metric** — an empty `filters` list is a definition, not a blank; a generated metric whose name another table defines is not added; nothing is dropped | Standing (2026-09-17) | [plans/semantic-layer-model.md](plans/semantic-layer-model.md) §4.6 |
| **A generation over chosen tables changes those tables and nothing else**, in every mode — the business context, exclusion rule and glossary are only filled where empty | Standing (2026-09-17) | [reference/semantic-layer.md](reference/semantic-layer.md) |
| ***Needs attention* is decided on the server from what is already stored** — no job, no model call, no table; the frontend only groups and words it, and excluded entities are never a reason | Standing (2026-09-17) | [plans/semantic-layer-model.md](plans/semantic-layer-model.md) §4.6 |
| **Layer text limits are checked on write, not on parse** — the editor and an import are refused, a generation is clipped, and a stored layer is never made unreadable by a limit | Standing (2026-09-17) | [reference/semantic-layer.md](reference/semantic-layer.md) |
