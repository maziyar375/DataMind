...
| # | Item | State |
| --- | --- | --- |
| 8 | **Hybrid retrieval** (B2) — embeddings over table and column names blended with exact match and FK expansion | **Not built.** `retrieve` still selects tables on word overlap — but as of 2026-09-22 those words include the semantic layer's business vocabulary (**A5**), so "churn" reaches the table a curator named. B2's embeddings are the remaining half, and **B1 no longer blocks it**: recall is measurable at `--retrieve-budget 8000` (80.2 % / 62.0 %, `dc2ea4fd`). *(The earlier note here called the knowledge-template matcher "A5"; that was A1's Phase 7. A5 is this row's synonym index.)* |
