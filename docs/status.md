...
| --- | --- | --- | --- |
| **Semantic layer: versions** — every save, generation, restore and delete is a numbered, attributed version with typed changes; writes carry a revision and a stale one is a 409; a generation merges into the current layer under a lock; History, per-entry history and restore in the editor; runs and benchmark runs record the version that answered, and *Grounded* reads it | 2026-09-15, Phase 1, migration `0032` | The running stack needs `make migrate`; version tables are not yet measured for size (§11 trigger) | [plans/semantic-layer-model.md](plans/semantic-layer-model.md) §4.2 |
