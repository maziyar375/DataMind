...
| 2026-08-01 | **UI polish pass + responsive reflow** (amends §7's mode split): the header toggle is now **"Edit grid"** and governs only the layout — drag, resize, add, the inline name — while a tile's *edit / duplicate / delete* moved to the tile's kebab in **both** modes, reachable without a mode switch. Also: an edit-mode alignment guide drawn at the dashboard's own cell geometry, new tiles sized per type so their content fits without inner scroll, charts re-fit on tile resize (a `ResizeObserver`; Vega only re-measures a `container` width on `window:resize`), the grid stacks to one column in reading order below 620px measured width (nothing written back), the sidebar collapses to an icon rail under 860px, and the settings drawer overlays under 900px. `typecheck` + `build` green; not verified visually (no browser here). |
