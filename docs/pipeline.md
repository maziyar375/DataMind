...
`NodeDeps.extra_rules`, and so is a **METRIC tile's** (`METRIC_SQL_RULES`, which
asks for the time series a big number needs before it can carry a delta or a
sparkline). `extra_rules` is empty for every other caller — empty meaning
byte-identical, since `_with_extra_rules` returns its input unchanged rather
than rebuilding it — which is why neither moves `PROMPT_VERSION`: a chat run
sends the same bytes it always did, and the eval suite drafts nothing. See
[pipeline-report.md §7](pipeline-report.md) and
[pipeline-dashboard.md §2 A1](pipeline-dashboard.md).

The rule that follows: **anything appended through `extra_rules` belongs to its
caller, not to the prompt.** Two callers now share the hook and their rules
compose rather than override (`_sql_rules_for`), because a rule that silently
replaced another would be found only by reading a prompt nobody prints.
