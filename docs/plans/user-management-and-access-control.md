...
| **semantic_layer** *(derived)* | a layer exists; when it was last written | read the published layer, its draft, and its history | edit the draft, publish it, restore a version, import, check an expression, queue a generation | delete the layer | grant/revoke |
