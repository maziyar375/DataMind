...
`npm test` runs **eighteen** suites with `node --experimental-strip-types`:
seventeen DOM-free logic modules under `src/components/` — `dashboard-schedule`,
`table-format`, `dashboard-document`, `palette`, `chat-format`,
`report-document`, `report-readiness`, `report-print`, `semantic-drift`,
`semantic-metrics`, `semantic-changes`, `semantic-score`, `knowledge-template`, `thinking`, `knowledge-queue`,
...
- **`npm test`** is not in CI. The eighteen suites above run only where somebody
  runs them.
