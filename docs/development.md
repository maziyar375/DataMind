...
`npm test` runs **twenty-one** suites with `node --experimental-strip-types`:
twenty DOM-free logic modules under `src/components/` — `dashboard-schedule`,
...
`provider-params`, `usage-chart`, `sections-model` — plus `scripts/permissions.test.ts`. They hold the logic
...
- **`npm test`** is not in CI. The twenty-one suites above run only where somebody
  runs them.
